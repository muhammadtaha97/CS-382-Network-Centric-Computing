...
if __name__ == '__main__':
    ...
    while True:
        ...
        window.addstr(0, 5, snake[0].score)
        ...
        #     snake[0].change_direction(event)
        if event in [KEY_UP]:
        	message ="0:"+ "up"
        ...
        checker =   False
        for body in snake[1].body_list[:-1]:
            if snake[0].head.coor == body.coor:
                checker = True

        if checker == True:
            break        
        if snake[0].head.x == MAX_X or snake[0].head.y == MAX_Y or snake[0].head.x == 0 or snake[0].head.y == 0:
            break
        if snake[0].collided:
            break
    curses.endwin()
